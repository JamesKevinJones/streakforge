// Adapted from a pasted rare-ui-style component. Restyled its two custom
// color tokens (SURFACE/RECESS, originally light-mode swatches with a dark
// variant) to this project's actual dark-glass tokens instead of carrying
// an unused light-mode branch. Wired into SettingsForm as "Delete Account"
// — the first real use for a destructive-confirm control in this app.
...
const SURFACE = "bg-white/8";
const RECESS = "bg-white/12";
const GLYPH = "text-white/60";
const FOCUS = "outline-none focus-visible:ring-2 focus-visible:ring-white/40";
const ACCENT = "#ff453a";

const LIFT = "glass";

const CIRCLE = `grid h-7 w-7 place-items-center rounded-full transition-colors duration-200 hover:bg-white/15 ${FOCUS} ${SURFACE} ${LIFT}`;
...
export function DeleteButton({
  ...
  return (
    <motion.div
      ...
      data-status={status}
      className={cn("glass relative h-12 rounded-2xl", GLYPH, className)}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ width: PANEL }}
            className={cn(
              "absolute inset-y-0 right-0 flex items-center justify-center gap-2 rounded-2xl px-2",
              RECESS,
            )}
            ...
            exit="hidden"
          >
            <span className="truncate text-xs font-semibold text-white/70">{confirmLabel}</span>
