// Adapted from a pasted rare-ui-style component, repurposed per the user's
// request as a "stress buster" widget (not a message-reaction picker,
// which this app has no surface for) — same mechanics, restyled surface
// color and wrapped by a labeled card in StreakCard's dashboard area.
          ...
          {open && (
            ...
            >
              <div
                ref={placeBar}
                role="menu"
                aria-label="Pick something to throw"
                aria-orientation="horizontal"
                onKeyDown={onMenuKeyDown}
                className={cn(
                  "glass relative flex items-center rounded-full",
                  s.pill,
                )}
              >
                {burst}
        ...
        <Trigger
          ...
          className={
            asChild
              ? undefined
              : cn(
                  "glass relative z-10 grid place-items-center rounded-full text-white/70 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40",
                  s.trigger,
                )
          }
        >
