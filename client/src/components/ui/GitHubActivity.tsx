// Adapted from a pasted rare-ui-style component. No Next.js-specific
// imports here (unlike GooeyNav) — this is copied essentially unchanged,
// just dropping the meaningless "use client" directive (a no-op outside
// Next's App Router) and confirming the `@/lib/utils` cn() import resolves
// via this project's Vite alias. Kept as .tsx: Vite/esbuild strips
// TypeScript syntax without needing the project to be a TS project.
...
const Tooltip = ({
  ...
  return createPortal(
    ...
    >
      <motion.div
        ref={ref}
        className="whitespace-nowrap rounded-lg bg-white px-2 py-1 text-[11px] font-medium text-black shadow-md"
      ...
      {showMonths && (
        ...
        >
          {toMonthLabels(visible).map((month, index) => (
            ...
            >
              {month && (
                <span className="absolute left-0 top-0 text-[10px] leading-none text-white/40">
                  {month}
                </span>
              )}
            </div>
...
const Avatar = ({
  ...
  <motion.span
    ...
    transition={transition}
    className={cn(
      "grid size-7 shrink-0 place-items-center overflow-hidden rounded-full bg-white/10 text-[11px] font-medium uppercase text-white/70 ring-2 ring-black",
...
const GitHubActivity = ({
  ...
  return (
    <div
      ...
      className={cn(
        "glass relative max-w-full overflow-hidden rounded-[28px] p-4",
      ...
      {...props}
    >
      <p className="mb-4 px-1.5 text-base font-medium text-white">
        {heading}
      </p>
      ...
      {repos.length > 0 && (
        <motion.div
          ...
          className={cn(
            "glass absolute inset-x-3 bottom-3 overflow-hidden",
            open && "top-3",
          )}
